...
def load_music(data_name, filename, n_bars, n_steps_per_bar,max_number):
    ...
    shape_pro = reduce(lambda a, b: a*b, data_binary.shape)
    shape_songs = shape_pro//2//16//84//4*2*4*16*84
    data_binary = data_binary.reshape(shape_pro,1)
    data_binary = data_binary[:shape_songs]                  
    data_binary = data_binary.reshape(shape_songs//2//16//84//4,2, 16, 84, 4)    
    #data_binary = data_binary.transpose([0,1,2, 4,3])
    return data_binary, data_ints
